...
/*   Updated: 2021/01/19 21:00:35 by ntomika          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_flags_123(int *flag, int l, int *r, char *cp)
{
	if (flag[1] == 1)
	{
		...
		*r += ft_putstr(cp);
		(flag[5] < 0) ? (*r += while_print(flag, flag[2] - flag[3] - 1, 4, ' ')) :
				(*r += while_print(flag, l, 4, ' '));
	}
	else
	{
		if (flag[3] <= l)
		{
			*r += while_print(flag, l + 1, 2, ' ');
			if (flag[5] < 0)
				*r += write(1, "-", 1);
		}
		else
